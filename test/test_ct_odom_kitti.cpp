        ...
        if (slam_node["odometry_options"]) {
            ...
            if (odometry_node["motion_compensation"]) {
                ...
                if (compensation == "NONE")
                    odometry_options.motion_compensation = NONE;
                    ...
                    odometry_options.motion_compensation = ITERATIVE;
            ...
            if (odometry_node["initialization"]) {
                ...
                if (initialization == "INIT_NONE")
                    odometry_options.initialization = INIT_NONE;
            ...
            if (odometry_node["ct_icp_options"]) {
                ...
                if (icp_node["viz_mode"]) {
                    ...
                    if (viz_mode == "NORMAL")
                        icp_options.viz_mode = NORMAL;
                    else if (viz_mode == "WEIGHT")
                        icp_options.viz_mode = WEIGHT;
                    else
                        icp_options.viz_mode = TIMESTAMP;
                }
...
int main(int argc, char **argv) {
    ...
    auto sequences = get_sequences(options.dataset_options);
    ...
    std::map<std::string, seq_errors> sequence_name_to_errors;
...
//#pragma omp parallel for num_threads(options.max_num_threads)
    for (int i = 0; i < num_sequences; ++i) { //num_sequences
        ...
        Odometry ct_icp_odometry(&options.odometry_options);
        ...
        if (has_ground_truth(options.dataset_options, sequence_id)) {
            ...
            seq_errors seq_error = eval(ground_truth_poses, trajectory_absolute_poses);
...

            {
                ...
                SaveMetrics(sequence_name_to_errors, options.output_dir + "metrics.yaml",
                                    valid_trajectory);
            };
        }
    }
